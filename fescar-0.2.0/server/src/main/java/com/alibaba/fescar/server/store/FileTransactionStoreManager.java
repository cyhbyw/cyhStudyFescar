...
public class FileTransactionStoreManager implements TransactionStoreManager {
    ...
    class WriteDataFileRunnable implements Runnable {
        ...
        public void run() {
            while (!stopping) {
                try {
                    TransactionWriteFuture transactionWriteFuture =
                            transactionWriteFutureQueue.poll(MAX_POOL_TIME_MILLS, TimeUnit.MILLISECONDS);
        ...
        private void saveHistory() throws IOException {
            try {
                List<GlobalSession> globalSessionsOverMaxTimeout = sessionManager
                        .findGlobalSessions(new SessionCondition(GlobalStatus.Begin, MAX_TRX_TIMEOUT_MILLS));
