...
public class ChannelManager {
     ...
     */
    private static final ConcurrentMap<String, ConcurrentMap<String, ConcurrentMap<String, ConcurrentMap<Integer, RpcContext>>>> RM_CHANNELS =
            new ConcurrentHashMap<String, ConcurrentMap<String, ConcurrentMap<String, ConcurrentMap<Integer, RpcContext>>>>();
    ...
    public static void registerTMChannel(RegisterTMRequest request, Channel channel)
        ...
        String clientIdentified =
                rpcContext.getApplicationId() + Constants.CLIENT_ID_SPLIT_CHAR + getClientIpFromChannel(channel);
    ...
    public static void registerRMChannel(RegisterRMRequest resourceManagerRequest, Channel channel)
        ...
        for (String resourceId : dbkeySet) {
            ...
            applicationIdMap.putIfAbsent(resourceManagerRequest.getApplicationId(),
                    new ConcurrentHashMap<String, ConcurrentMap<Integer, RpcContext>>());
            ConcurrentMap<String, ConcurrentMap<Integer, RpcContext>> clientIpMap =
                    applicationIdMap.get(resourceManagerRequest.getApplicationId());
    ...
    private static void updateChannelsResource(String resourceId, String clientIp, String applicationId) {
        ...
        for (ConcurrentMap.Entry<String, ConcurrentMap<String, ConcurrentMap<String, ConcurrentMap<Integer, RpcContext>>>> rmChannelEntry : RM_CHANNELS
                .entrySet()) {
    ...
    public static Channel getChannel(String resourceId, String clientId) {
                ...
                if (resultChannel == null) {
                    for (ConcurrentMap.Entry<Integer, RpcContext> portMapOnTargetIPEntry : portMapOnTargetIP
                        ...
                        if (channel.isActive()) {
                            resultChannel = channel;
                            if (LOGGER.isInfoEnabled()) {
                                LOGGER.info("Choose " + channel + " on the same IP[" + targetIP
                                        + "]  as alternative of " + clientId);
                            }
                            break;
                        } else {
        ...
        if (resultChannel == null) {
            ...
            if (resultChannel == null) {
                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info(
                            "No channel is available for resource[" + resourceId + "]  as alternative of " + clientId);
                }
            } else {
                if (LOGGER.isInfoEnabled()) {
