...
public class AsyncWorker implements ResourceManagerInbound {
    ...
    private static final List<Phase2Context> ASYNC_COMMIT_BUFFER =
            Collections.synchronizedList(new ArrayList<Phase2Context>());

    private static int ASYNC_COMMIT_BUFFER_LIMIT =
            ConfigurationFactory.getInstance().getInt(CLIENT_ASYNC_COMMIT_BUFFER_LIMIT, 10000);
            ...
            try {
                ...
                for (Phase2Context commitContext : contextsGroupedByResourceId) {
                    try {
                        ...
                        LOGGER.warn(
                                "Failed to delete undo log [" + commitContext.branchId + "/" + commitContext.xid + "]",
                                ex);
                    }
                }
