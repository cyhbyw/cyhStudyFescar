    ...
    private static class MyFailureHandler implements FailureHandler {
        ...
        public void onCommitFailure(final GlobalTransaction tx, Throwable cause) {
            ...
            final ScheduledExecutorService schedule =
                    new ScheduledThreadPoolExecutor(1, new NamedThreadFactory("BusinessRetryCommit", 1, true));
        ...
        public void onRollbackFailure(final GlobalTransaction tx, Throwable cause) {
            ...
            final ScheduledExecutorService schedule =
                    new ScheduledThreadPoolExecutor(1, new NamedThreadFactory("BusinessRetryRollback", 1, true));
            schedule.scheduleAtFixedRate(new Runnable() {
                @Override
                public void run() {
                    try {
