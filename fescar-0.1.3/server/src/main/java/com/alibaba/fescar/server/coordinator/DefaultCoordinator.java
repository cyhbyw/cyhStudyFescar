...
public class DefaultCoordinator extends AbstractTCInboundHandler
    ...
    protected void doBranchRegister(BranchRegisterRequest request, BranchRegisterResponse response,
        ...
        response.setBranchId(core.branchRegister(request.getBranchType(), request.getResourceId(),
                rpcContext.getClientId(), XID.generateXID(request.getTransactionId()), request.getLockKey()));
    ...
    private void timeoutCheck() throws TransactionException {
        ...
        if (size > 0 && LOGGER.isDebugEnabled()) {
            LOGGER.debug("Transaction Timeout Check Begin. size: " + size);
        }
        for (GlobalSession globalSession : allSessions) {
            long xid = globalSession.getTransactionId();
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Timeout check for global session. xid: " + xid + ", status: " + globalSession.getStatus()
                        + ", beginTime: " + globalSession.getBeginTime() + ", timeout: " + globalSession.getTimeout());
            }
            ...
            LOGGER.info("Global transaction[" + xid + "] is timeout and will be rolled back.");

            globalSession.addSessionLifecycleListener(SessionHolder.getRetryRollbackingSessionManager());
            SessionHolder.getRetryRollbackingSessionManager().addGlobalSession(globalSession);
        }
        if (size > 0 && LOGGER.isDebugEnabled()) {
            LOGGER.debug("Transaction Timeout Check End. size: " + size);
        }
    }
    ...
    private void handleRetryRollbacking() {
        ...
        for (GlobalSession rollbackingSession : rollbackingSessions) {
            try {
                ...
                LOGGER.info("Failed to retry rollbacking [{}] {} {}", rollbackingSession.getTransactionId(),
    ...
    private void handleRetryCommitting() {
        ...
        for (GlobalSession committingSession : committingSessions) {
            try {
                ...
                LOGGER.info("Failed to retry committing [{}] {} {}", committingSession.getTransactionId(), ex.getCode(),
                        ex.getMessage());
            }
        }
    }
    ...
    private void handleAsyncCommitting() {
        ...
        for (GlobalSession asyncCommittingSession : asyncCommittingSessions) {
            try {
                ...
                LOGGER.info("Failed to async committing [{}] {} {}", asyncCommittingSession.getTransactionId(),
                        ex.getCode(), ex.getMessage());
            }
        }
    }

    private ScheduledThreadPoolExecutor retryRollbacking =
            new ScheduledThreadPoolExecutor(1, new NamedThreadFactory("RetryRollbacking", 1));

    private ScheduledThreadPoolExecutor retryCommitting =
            new ScheduledThreadPoolExecutor(1, new NamedThreadFactory("RetryCommitting", 1));

    private ScheduledThreadPoolExecutor asyncCommitting =
            new ScheduledThreadPoolExecutor(1, new NamedThreadFactory("AsyncCommitting", 1));

    private ScheduledThreadPoolExecutor timeoutCheck =
